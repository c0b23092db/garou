...
pub(super) fn pan_image(redraw_mode: &mut RedrawMode, state: &mut ViewerState, dx: i16, dy: i16) {
    if let Some((x, y, _, _)) = state.last_image_rect() {
        let base_x = if state.sidebar_visible() {
            i32::from(state.sidebar_size())
        } else {
            0
        };

        let mut pan_x = i32::from(state.pan_x());
        let mut pan_y = i32::from(state.pan_y());
        ...
        if pan_x != state.pan_x() || pan_y != state.pan_y() {
